...
test('snapshotToDisplay normalizes cutoff back to 0..1 (log inverse)', () => {
  const lo = snapshotToDisplay({ filter: { cutoff: 80, q: 1 }, reverb: { wet: 0, active: false }, delay: { mix: 0, time: 0.28, feedback: 0, active: false }, tremolo: { rate: 0.1, depth: 0, active: false }, compressor: { amount: 0, threshold: 0, ratio: 1, active: false } });
  const hi = snapshotToDisplay({ filter: { cutoff: 12000, q: 1 }, reverb: { wet: 1.3, active: true }, delay: { mix: 0.5, time: 0.28, feedback: 0.55, active: true }, tremolo: { rate: 12, depth: 1, active: true }, compressor: { amount: 1, threshold: -40, ratio: 12, active: true } });
